...
const sendResetLink = async (email, id) => {
    try {
        ...
        //remove request object created in DB after 10 min 
        if (success) {
            setTimeout(async () => {
                try {
...
const handleForgotPassword = async (req, res) => {
    try {
        ...
        // Create temp object in DB that can be cross referenced with temp ID sent to user (deleted after 10 minutes - see sendResetLink)
        const tempId = uuidv4()
